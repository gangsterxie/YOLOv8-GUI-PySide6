...
class YoloPredictor(BasePredictor, QObject):
    ...
    def run(self, *args, **kwargs):
        try:
            ...
            self.yolo2main_status_msg.emit('Loading model...')  # Emit model loading status
            self.initialize_model()  # Initialize model

            with self._lock:  # Ensure thread-safe inference
                if self.task == 'Track':
                    track_history = defaultdict(lambda: [])
                else:
                    track_history = None

                self.setup_source(self.source or self.args.source)  # Setup source
                self.check_save_dirs()  # Check save directories

                if not self.done_warmup:
                    self.model.warmup(imgsz=(1 if self.model.pt or self.model.triton else self.dataset.bs, 3, *self.imgsz))  # Model warmup
                ...
                while True:
                    ...
                    if self.continue_dtc:
                        try:
                            ...
                            break
                        self.batch = batch
                        self.yolo2main_status_msg.emit('Detecting...')  # Emit detection status

                        paths, im0s, s = self.batch
                        im = self.preprocess_batch(im0s, profilers[0])  # Preprocess batch
                        preds = self.inference(im, profilers[1], *args, **kwargs)  # Inference
                        if self.task == 'Track':
                            self.results, self.track_pointlist = self.postprocess_results(preds, im, im0s, track_history, profilers[2])  # Post-process results
                        else:
                            self.results = self.postprocess_results(preds, im, im0s, track_history, profilers[2])

                        self.run_callbacks('on_predict_postprocess_end')
                        self.handle_results(im, im0s, paths, s, profilers)  # Handle results

                        self.yolo2main_progress.emit(self.progress_value)  # Emit progress

                    if self.check_completion():  # Check completion
                        break
                ...
                if not self.source_type.stream and self.frames is None or self.frame is None:
                    self.yolo2main_status_msg.emit('Detection complete')  # Emit completion status

        except Exception as e:
            traceback.print_exc()
            print(f"Error: {e}")
            self.yolo2main_status_msg.emit('%s' % e)  # Emit error status

    def initialize_model(self):
        if self.task == 'Track':
            self.track_model = YOLO(self.new_model_name)  # Initialize tracking model
        self.setup_model(self.new_model_name)  # Setup model
        self.used_model_name = self.new_model_name

    def check_model_update(self):
        if self.used_model_name != self.new_model_name:
            if self.task == 'Track':
                self.track_model = YOLO(self.used_model_name)  # Update tracking model
            self.setup_model(self.new_model_name)  # Setup new model
            self.used_model_name = self.new_model_name

    def preprocess_batch(self, im0s, profiler):
        with profiler:
            return self.classify_preprocess(im0s) if self.task == 'Classify' else self.preprocess(im0s)  # Preprocess batch

    def postprocess_results(self, preds, im, im0s, track_history, profiler):
        with profiler:
            if self.task == 'Classify':
                return self.classify_postprocess(preds, im, im0s)  # Classification post-processing
            elif self.task == 'Track':
                return self.track_postprocess(self.track_model, track_history, preds, im, im0s)  # Tracking post-processing
            else:
                postprocess_methods = {
                ...
                return postprocess_methods[self.task](preds, im, im0s)  # Other task post-processing
    ...
    def handle_results(self, im, im0s, paths, s, profilers):
        ...
        for i in range(n):
            ...
            self.results[i].speed = {
                'preprocess': profilers[0].dt * 1E3 / n,  # Pre-processing time
                'inference': profilers[1].dt * 1E3 / n,  # Inference time
                'postprocess': profilers[2].dt * 1E3 / n  # Post-processing time
            }

            self.class_nums = 0
            self.target_nums = 0
            s[i] += self.write_results(i, Path(paths[i]), im, s)  # Write results
            im0 = None if self.source_type.tensor else im0s[i].copy()
            if 'no detections' in s:
                self.im = im0

            self.update_progress()  # Update progress
            self.send_results(im0)  # Send results

    def update_progress(self):
        if isinstance(self.frame, int) and (not isinstance(self.frames, list) and self.frames is not None):
            self.progress_value = int(self.frame / self.frames * 1000)  # Calculate progress value
        elif not self.source or self.frames is None or self.frame is None:
            self.progress_value = int(1000)  # Set progress value to 1000

    def send_results(self, im0):
        self.yolo2main_pre_img.emit(im0 if isinstance(im0, np.ndarray) else im0[0])  # Send pre-processed image
        self.yolo2main_res_img.emit(self.im)  # Send result image
        if self.task != 'Classify':
            self.yolo2main_class_num.emit(self.class_nums)  # Send class numbers
            self.yolo2main_target_num.emit(self.target_nums)  # Send target numbers
        if not isinstance(self.frames, list) and self.frames is not None:
            self.yolo2main_fps.emit(str(self.fps))  # Send FPS
        if self.speed_thres != 0:
            time.sleep(self.speed_thres / 1000)  # Delay

    def check_completion(self):
        if (self.frame == self.frames) and self.frames is not None and self.frame is not None:
            self.release_video_writers()  # Release video writers
            self.yolo2main_status_msg.emit('Detection complete')  # Send completion status
            return True
        return False
    ...
    def release_video_writers(self):
        for v in self.vid_writer.values():
            if isinstance(v, cv2.VideoWriter):
                v.release()  # Release video writers
