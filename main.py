...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def update_model_lists(self):
        ...
        self.pt_list.sort(key=lambda x: os.path.getsize(os.path.join(model_dir, x)))  # 按文件大小排序
        self.model_box.clear()
        self.model_box.addItems(self.pt_list)
        self.yolo_predict.new_model_name = os.path.join(model_dir, self.select_model)
        # 讀取 cam 模型文件夾
        self.pt_list_cam = self.pt_list.copy()  # 使用相同的文件列表
    ...
    def reset_yolo_thread(self):
        ...
        self.yolo_thread.wait()  # 等待線程完成

        self.yolo_predict.deleteLater()  # 銷毀當前對象
        self.yolo_predict = YoloPredictor()                           # 創建 YOLO 實例
    ...
    def reset_yolo_thread_cam(self):
        ...
        self.yolo_thread_cam.wait()  # 等待線程完成

        self.yolo_predict_cam.deleteLater()  # 銷毀當前對象
        self.yolo_predict_cam = YoloPredictor()                           # 創建 YOLO 實例
