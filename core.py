...
class YoloPredictor(BasePredictor, QObject):
        ...
        try:
            ...
            if is_folder:
                for source in self.source:
                    self.setup_source(source)  # 設置來源
                    self.stream_inference()
            else:
                self.setup_source(self.source)  # 設置來源
                self.stream_inference()
    ...
    def stream_inference(self, *args, **kwargs):
        try:
            ...
            while True:
                ...
                if self.continue_dtc:
                    try:
                        ...
                        break
                    self.batch = batch
                    self.yolo2main_status_msg.emit('檢測中...')  # Emit detection status

                    paths, im0s, s = self.batch
                    im = self.preprocess_batch(im0s, profilers[0])  # 預處理批次
                    preds = self.inference(im, profilers[1], *args, **kwargs)  # 推論
                    if self.task == 'Track':
                        self.results, self.track_pointlist = self.postprocess_results(preds, im, im0s, self.track_history, profilers[2])  # 後處理結果
                    else:
            ...
            if not self.source_type.stream and self.frames is None or self.frame is None:
                self.yolo2main_status_msg.emit('檢測完成')  # Emit completion status
    ...
    def initialize_model(self):
        if not self.model_initialized:
            ...
            self.model_initialized = True  # 更新標誌
